...
// 唯一例外:VLM 的 apiKey 走 .env.<环境> 的 TESTER_VLM_API_KEY(密钥不进仓库)。
...
export const testerConfig = {
  ...
  //   login     是否登录
  //   vlm       视觉兜底(enabled/model/apiUrl/timeout),apiKey 走 .env.<环境> 的 TESTER_VLM_API_KEY
  envs: {
    // 开发:本地+Chrome
    dev: {
      ...
      vlm: {
        enabled: false,
        model: "glm-4v",
        apiUrl: "https://你的模型服务地址/v1/chat/completions",
        timeout: 8,
      },
    },
    // 测试(默认环境)
