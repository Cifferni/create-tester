...
- **需要干净/特殊环境的用例:先构造、再 skip**。如"空状态"用例,先尝试构造环境(调 `env_reset` / 通过应用 UI 或 API 清理或造数据),构造得了就跑,构造不了才 `test.skip` + 说明。
- **构造环境可以试,但不能瞎搞**:
  - 只动**测试产生的数据**,绝不碰真实/生产数据
  - 先探查确认应用有清理/造数手段(接口或 UI),有明确路径才动手
  - 清理要可预期、可还原(记下改了啥,能恢复)
  - 拿不准就直接 skip + 说明,不要冒险改数据
- **环境数据敏感**:用例尽量"造数据 + 自我清理";判断"新增/删除"用计数对比(namesBefore/namesAfter),不要靠名字唯一。`run_tests` 传 `{workers:N}` 提速前,先确认用例彼此隔离。
- **环境可复跑**:测试会改被测数据时,在 `mcp/env-reset.cjs` 里写清理逻辑(调用后端清理接口),跑回归前调 `env_reset` 还原环境,保证连续两次结果可比。
