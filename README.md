...
生成的测试项目会把引擎 [**tester-core**](https://github.com/Cifferni/tester-core) 作为依赖安装,项目干净(只有 `cases/`、`config.ts`、脚本)。
...
```bash
cd my-test && npm install     # 自动装引擎 tester-core + 自动下载浏览器
...
├── package.json    脚本 + 依赖 tester-core
├── reports/        运行报告(自动生成)
└── data/           抓包记录、截图(自动生成)
```
